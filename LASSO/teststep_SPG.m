...
[x0, its0, ek0, fk0, sk0, gk0] = func_SPG_const(para, iGradF, ObjF);
...
[x1, its1, ek1, fk1, sk1, gk1] = func_SPG(para, iGradF, ObjF);
...
% axis([1, its1/m, 0 1.1*gk1(end)]);

ylabel({'$\eta_{k}$'}, 'FontSize', labelFontSize, 'FontAngle', 'normal', 'Interpreter', 'latex');
xlabel({'\vspace{-0.0mm}';'$Iterations$'}, 'FontSize', labelFontSize, 'FontAngle', 'normal', 'Interpreter', 'latex');
% 
...
lg = legend([p0,p1],...
    sprintf('{Prox-SG constant stepsize}'),sprintf('{Prox-SG change stepsize}'));
...
epsname = sprintf('SPG_teststep_%s_gamma.%s', filename, outputType);
...
epsname = sprintf('SPG_testing_value%s_objf.%s', filename, outputType);
if strcmp(outputType, 'png')
    print(epsname, '-dpng');
else
    print(epsname, '-dpdf');
